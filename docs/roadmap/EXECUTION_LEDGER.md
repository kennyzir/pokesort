...
| 4 — Archive and dated pages | CONDITIONAL PASS — 2026-08-24 | Preserved a static, month-grouped 31-date Archive with exact content, navigation, breadcrumbs, progressive hints, and `noindex,follow` publication hold. The edge redirect window is pinned to the actual build artifact. Added a GitHub schedule at 00:05 UTC that runs the static Gate and pushes a dated refresh commit to trigger the existing Cloudflare Pages Git deployment. | `npm test`; 31 exact cards/routes; production in-window query 308 to an existing static route; outside-window query 200 compatibility; newest+1/oldest−1 and invalid date edge regressions; production Chrome Archive count 31 | GSC evidence for an index release is unavailable. Scheduled workflow and downstream Cloudflare deployment require first-run verification. | Manually dispatch and verify the workflow plus Cloudflare check; keep dated index release held until packs are materially unique |
