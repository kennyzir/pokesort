...
Cloudflare KV request volume, storage quota, Pages/Workers plan limits, billing cost, and the authorized target account's exact entitlement remain **UNVERIFIED**. The repository makes no zero-cost or sufficient-quota claim; R8 must verify these account-specific assumptions before enabling either edge flag.
