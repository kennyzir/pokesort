...
| Resolved | game data | Legacy runtime selected one of three packs for both Daily and Infinite | historical `GROUPS.length === 3` selector | Repetition and thin dated pages | Replaced with 365 immutable Daily manifests and a separate 1,000-board lazy-loaded Infinite pool; legacy packs remain fixtures only | `data/puzzles/`; `scripts/puzzle/`; `assets/game.js` |
