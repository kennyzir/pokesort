...
await writeFile(new URL("sitemap.xml", output), `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${routes.map((route) => { const date = /^\/daily\/(\d{4}-\d{2}-\d{2})\/$/.exec(route)?.[1]; const lastmod = date || archiveLastmods.get(route); return `  <url><loc>${siteUrl}${route}</loc>${lastmod ? `<lastmod>${lastmod}</lastmod>` : ""}</url>`; }).join("\n")}\n</urlset>\n`);
