...
export async function validatePublicDailyHistory({
  ...
  assert(names.length === index.entries.length, "Public Daily history file/index count mismatch");
  ...
  for (const [position, entry] of index.entries.entries()) {
    ...
    assert(entry.file === `${entry.date}.json` && names[position] === entry.file, `Public Daily history gap or filename mismatch: ${entry.date}`);
  ...
  return { directory, index, manifests, dates: manifests.map(({ date }) => date), oldestDate: manifests[0].date, newestDate: manifests.at(-1).date };
}
