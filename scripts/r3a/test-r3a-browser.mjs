...
async function installCapture(context) {
  await context.addInitScript(() => {
    ...
    Object.defineProperty(globalThis, "gtag", { configurable: true, get: () => (...args) => { if (args[0] === "event") globalThis.__pokesortEvents.push({ name: args[1], parameters: args[2] }); }, set: () => {} });
  });
...
try {
  ...
  record("/", "1440x900", "solve four intended groups", "correct history, group events and one solved completion", `groups=${current.solved.length}; completion=${current.analyticsCompletionRecorded}`);
  ...
  const shareCount = (await events(page)).filter(({ name }) => name === "pokesort_share").length;
  ...
  record("/", "1440x900", "share success then cancel native share", "clipboard success event; cancellation no event", `share_events=${shareCount}`);
  ...
  assert.equal((await state(failurePage)).history.length, 20); assert.equal((await state(failurePage)).mistakes, 0);
  ...
  assert.equal((await events(failurePage)).filter(({ name, parameters }) => name === "pokesort_game_complete" && parameters.outcome === "failed").length, 1);
  record("/", "1440x900", "history cap and four invalid guesses", "20 newest history items; third stays active; fourth fails once", "history=20; mistakes=4; failed_events=1"); await failureContext.close();
  ...
  record("/", "1440x900", "Daily migration and corrupt state recovery", "legal legacy migrates; invalid JSON resets; off-board history ignored", "legacy_mistakes=2; reset=0; history=1"); await migrationContext.close();
  ...
  assert.equal((await events(revealPage)).filter(({ name }) => name === "pokesort_reveal").length, 1); assert.equal((await events(revealPage)).filter(({ name, parameters }) => name === "pokesort_game_complete" && parameters.outcome === "revealed").length, 1);
  record("/", "1440x900", "Reveal after hint", "Reveal remains independent and records one revealed completion", "revealed=true; hint_preserved=1"); await revealContext.close();
  ...
  await archivePage.screenshot({ path: join(evidenceDirectory, "desktop-archive.png"), fullPage: true }); await assertEventPrivacy(archivePage);
  ...
  assert.equal((await events(infinitePage)).filter(({ name }) => name === "pokesort_new_infinite").length, 1);
  record("/infinite/", "1440x900", "sidecar overlap and New Infinite", "no penalty and one user-initiated new event", `mistakes=0; new_events=1`);
  ...
  await infinitePage.screenshot({ path: join(evidenceDirectory, "desktop-infinite.png"), fullPage: true }); await assertEventPrivacy(infinitePage);

  const homeHtml = await readFile(resolve(dist, "index.html"), "utf8");
  for (const scenario of ["missing", "stale", "invalid"]) {
    ...
    if (scenario === "invalid") body = body.replace(/(<script id="pokesort-puzzle-data" type="application\/json">)[\s\S]*?(<\/script>)/, "$1{$2");
    if (scenario === "stale") body = body.replace(/"date":"2026-09-01"/, '"date":"2026-08-31"');
    ...
    record("/", "1440x900", `${scenario} embedded fallback`, "request API once then fail closed", `requests=${requests}; state=unavailable`); await context.close();
  }
  ...
  await staleApiPage.route("**/api/daily/current", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ schemaVersion: 1, status: "ready", utcDate: "2026-08-31", manifest: {} }) }));
  ...
  record("/", "1440x900", "API returns yesterday", "fail closed and show no stale board", "state=unavailable; cards=0"); await staleApiContext.close();
  ...
  for (const scenario of ["missing", "hash", "puzzle-id"]) {
    ...
    if (scenario === "puzzle-id") {
      const mutatedShard = structuredClone(originalOverlapShard); mutatedShard.puzzles[poolIndex - sourceEntry.start].puzzleId = "infinite-wrong"; const hashedShard = withHash(mutatedShard);
    ...
    record("/infinite/", "1440x900", `${scenario} overlap contract`, "fail closed with safe infinite_overlap_contract event", `state=unavailable; stage=${loadErrors.at(-1).parameters.error_stage}`); await context.close();
  }

  await desktop.tracing.stop({ path: join(evidenceDirectory, "desktop-trace.zip") }); await desktop.close();
  ...
  await mobile.tracing.stop({ path: join(evidenceDirectory, "mobile-trace.zip") }); await mobile.close();
