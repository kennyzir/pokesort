...
if (!howTo.includes("How to play PokeSort 4×4") || howTo.includes("precise Pokédex connection") || howTo.includes("regional forms, or another verifiable Pokédex trait")) throw new Error("How-to content must identify the 4×4 mechanic and describe only reviewed Pokémon connection families");
...
if (!infinitePage.includes("Play unlimited 4×4 Pokémon sorting puzzles") || !infinitePage.includes("does not claim endless unique source data")) throw new Error("Infinite must expose its distinct 4×4 task and finite-pack boundary");
if (!categoriesPage.includes("PokeSort 4×4 category reference") || !categoriesPage.includes("reviewed category families used by current PokeSort 4×4 boards")) throw new Error("Categories metadata/body must identify the reviewed 4×4 reference task");
...
if ((archive.match(/<section class="archive-month">/g) || []).length !== archiveMonths.size || [...archiveMonths].some((month) => !archive.includes(`<time datetime="${month}">`))) throw new Error("Archive dates must be grouped under semantic month headings");
...
const expectedPaths = ["/", "/infinite/", "/archive/", "/how-to-play/", "/categories/", "/pokelike-pokesort/", "/about/", "/pokesort-down/", "/privacy/"];
...
if (urls.some((url) => /\/daily\/(\d{4}-\d{2}-\d{2})\//.exec(url)?.[1] > today)) throw new Error("Sitemap contains a future daily page");

const titles = new Map(), descriptions = new Map(), knownPaths = new Set([...urls.map((url) => new URL(url).pathname), ...archiveLinks.map((date) => `/daily/${date}/`), "/pokesort-alternative/"]);
const mainTexts = [];
for (const url of urls) {
  ...
  if (/noindex/i.test(html)) throw new Error(`${pathname} is in the sitemap but noindexed`);
...
if (indexableDailyCount !== 0) throw new Error("Dated index release must remain held while the rolling window repeats only three packs");
for (const date of archiveLinks) {
...
console.log(`SEO validation passed for ${urls.length} canonical, indexable routes.`);
